...
class QoSSubscriber(Node):
    def __init__(self):
        super().__init__('qos_subscriber1')
        # We create a QoSProfile object with a depth of 10 and a reliability policy of RELIABLE
        qos_profile = QoSProfile(depth=10, reliability=ReliabilityPolicy.RELIABLE)
        self.subscription = self.create_subscription(Float32, 'topic', self.listener_callback, qos_profile)  # Here we pass the QoSProfile object to the create_subscription method

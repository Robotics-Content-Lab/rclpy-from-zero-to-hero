...
class KalmanFilter:
    ...
    def predict(mu, P, u, A, B, Q, dt):
        ...
        """
        assert mu.shape == (STATE_DIM, 1), "x must be a 1x1 matrix"
        ...
        x_pred = A @ mu + B @ u  # Predict the state
    ...
    def update(mu_pred, P_pred, z, C, R):
        ...
        """
        assert mu_pred.shape == (STATE_DIM, 1), "mu_pred must be a 1x1 matrix"
...
class KalmanFilterNode(Node):
    ...
    def run_kalman_filter(self):
        ...
        with self.lock:  # Lock to protect shared variables (self.u, self.z, self.mu, self.P)
